...
export function RadarOwnerSovereignDashboard() {
  ...
  return (
    <div className="space-y-8 bg-radar-black text-right p-6 rounded-3xl border border-red-500/10 min-h-screen text-white font-sans" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-red-500/20 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Badge className="bg-radar-danger hover:bg-radar-danger/90 text-white font-black px-3 py-1 text-[11px] rounded-full animate-pulse shadow-[0_0_15px_rgb(var(--radar-danger-rgb)/0.4)] font-sans">
              لوحة المالك  ● V5.5 SECURITY PROTOCOL
            </Badge>
          </div>
          <h2 className="text-2xl sm:text-3xl font-black text-radar-neon tracking-tight flex items-center gap-2 font-sans">
            <ShieldAlert className="w-8 h-8 text-radar-danger animate-pulse animate-duration-1000" />
        ...
        <div className="bg-zinc-950/90 border border-radar-neon/20 rounded-2xl p-4 min-w-[240px] text-right">
          <span className="text-[10px] text-gray-500 block font-bold">إجمالي الاحتياطي المالي المحصن لأصحاب الحقوق</span>
          <span className="text-2xl font-black text-radar-neon font-mono block mt-1">{(auditedStats.totalNet).toFixed(2)} د.أ</span>
      ...
      <Card className="bg-radar-black border border-blue-500/15 shadow-[0_8px_32px_rgba(0,0,0,0.5)] rounded-2xl overflow-hidden mt-8">
        ...
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-right" dir="rtl">
            ...
            {/* Sectors List */}
            <div className="space-y-4">
              <h3 className="text-xs font-black text-radar-neon tracking-wide border-b border-white/5 pb-2">🧱 عزل القطاعات والقطوعات (Domain Sectors)</h3>
              <div className="space-y-3 font-sans">
                {Object.values(SovereignDemarcationCatalog.sectors).map(sector => (
                  <div key={sector.id} className="bg-zinc-950 p-4 rounded-xl border border-white/5 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge className="bg-radar-forest-deep/50 border border-radar-neon/30 text-radar-neon text-[9px] font-mono px-2 py-0.5">{sector.id}</Badge>
                    </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-white/5">
                      <div className="grid grid-cols-2 gap-3 text-[10px]">
                        ...
                        <div className="bg-zinc-900/40 p-2 rounded-lg border border-white/5 space-y-1">
                          <span className="text-[9px] text-radar-neon font-black block">💻 الواجهة الأمامية (Front-End):</span>
                          <div>
