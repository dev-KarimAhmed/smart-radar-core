...
export function RiderPortal() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6 pb-10 text-right" dir="rtl">
      <header className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-radar-teal/20 bg-radar-card/70 backdrop-blur-xl p-5 shadow-xl sm:flex-row sm:items-center">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <span className="rounded-xl bg-radar-teal/10 p-1.5 text-radar-teal">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="border-radar-teal/30 bg-radar-teal/5 px-2.5 py-1 text-[9px] text-radar-teal">
            {t('protocol')}
          </Badge>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
        <section className="space-y-6 md:col-span-5">
          <Card className="rounded-2xl border border-radar-teal/20 bg-radar-card/80 backdrop-blur-xl shadow-xl">
            <CardHeader className="border-b border-white/10 p-5 pb-3">
              <CardTitle className="flex items-center gap-2 text-sm font-black text-white">
                <MapPin className="h-4 w-4 text-radar-teal" />
                {t('routeCard')}
              </CardTitle>
            ...
            <CardContent className="space-y-4 p-5">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="w-full rounded-xl border-white/10 bg-black/60 pr-3 text-right text-xs text-white focus:border-radar-teal"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="w-full rounded-xl border-white/10 bg-black/60 pr-3 text-center font-mono text-xs text-white focus:border-radar-teal"
                />
              </div>

              <div className="space-y-3 rounded-xl border border-radar-teal/25 bg-radar-teal/5 p-4">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-gray-400">{t('pricing')}</span>
                  {frozenPrice ? (
                    <Badge variant="outline" className="border-radar-teal font-mono text-radar-teal">
                      {frozenH3 || "H3"}
                    </Badge>
                  ) : (
                ...
                <Button
                  ...
                  className="flex h-9 w-full items-center justify-center gap-1.5 rounded-lg border border-radar-teal/40 bg-radar-teal/10 py-2 text-xs font-bold text-radar-teal hover:bg-radar-teal/20"
                >
        ...
        <section className="space-y-6 md:col-span-7">
          <Card className="flex flex-1 flex-col rounded-2xl border border-radar-teal/20 bg-radar-card/80 backdrop-blur-xl shadow-xl">
            <CardHeader className="flex flex-row items-center justify-between gap-4 border-b border-white/10 p-5">
              <div>
                <CardTitle className="flex items-center gap-2 text-sm font-black text-white">
                  <Search className="h-4 w-4 text-radar-teal" />
                  {t('nearbyDrivers')}
                </CardTitle>
              ...
              <Button
                ...
                disabled={isScanning}
                className="flex h-8 items-center gap-1 rounded-lg border border-radar-teal/35 bg-black/40 px-2.5 text-xs text-radar-teal"
              >
            ...
            <CardContent className="max-h-[440px] space-y-4 overflow-y-auto p-5">
              {isScanning ? (
                <div className="space-y-3 py-12 text-center">
                  <RefreshCw className="mx-auto h-8 w-8 animate-spin text-radar-teal" />
                  ...
                  {nearbyDrivers.map((driver) => (
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-radar-teal/20 bg-radar-teal/10 font-bold text-radar-teal">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className="border border-radar-teal/20 bg-radar-teal/10 px-2 py-0.5 text-[8px] font-bold leading-none text-radar-teal hover:bg-radar-teal/15">
                              {driver.rank}
                            </Badge>
                          </div>
                      ...
                      <Button
                        ...
                        className={cn(
                          ...
                          isLocked
                            ...
                            : "bg-radar-teal text-black hover:bg-radar-teal/90",
                        )}
                      >
                        {isLocked ? (
                          <>
      ...
      <div className="flex items-start gap-2 rounded-xl border border-radar-teal/25 bg-radar-card/60 p-4 text-right text-[10px] leading-normal text-gray-400 shadow-sm">
        <Shield className="mt-0.5 h-4 w-4 shrink-0 text-radar-teal" />
        <p>{t('safety')}</p>
      </div>
    </div>
  );
