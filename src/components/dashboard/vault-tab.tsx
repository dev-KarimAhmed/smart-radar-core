  ...
  const handleZeroClickAction = (actionType: 'call' | 'whatsapp', contactNumber: string, title?: string) => {
    ...
    if (actionType === 'whatsapp') {
      const text = encodeURIComponent(
        `مرحباً، شاهدت إعلانكم "${title || ''}" في خزنة الرائد العظمى وأود الاستفسار عن تفاصيل العرض.`
      );
      ...
      {savedAds.length === 0 ? (
        <div className="bg-neutral-900/35 border border-white/5 text-center py-16 px-6 rounded-2xl flex flex-col items-center justify-center gap-4">
          ...
          <div className="space-y-1">
            <p className="text-sm font-bold text-white">الخزنة السيادية فارغة حالياً</p>
            <p className="text-xs text-gray-400 max-w-xs leading-relaxed mx-auto">
              اضغط على أيقونة القلب الأخضر الزمردي أعلى أي إعلان ميداني وسيتم حفظ بيانات تواصل عروض الأباطرة والخصومات فوراً في هذه المساحة المحصنة.
            </p>
          </div>
        </div>
      ) : (
