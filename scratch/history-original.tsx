...
const UNAVAILABLE_AR = '╪║┘è╪▒ ┘à╪ز╪د╪ص';
const LOCAL_RIDER_AR = '╪▒╪د┘â╪ذ';
const LOCAL_LOCATION_AR = '┘à┘ê┘é╪╣┘è ╪د┘╪ص╪د┘┘è';
...
function formatVehicleInfo(vehicle: any) {
  if (!vehicle || typeof vehicle !== 'object') return '╪║┘è╪▒ ┘à╪ز╪د╪ص';
  const parts = [
    vehicle.make,
  ...
  return parts.length > 0 ? parts.join(' - ') : '╪║┘è╪▒ ┘à╪ز╪د╪ص';
}
...
const VEHICLE_CRITERIA_LABELS: Record<string, string> = {
  cleanliness: '┘╪╕╪د┘╪ر ╪د┘╪╡╪د┘┘ê┘',
  ac: '╪╣┘à┘ ╪د┘╪ز┘â┘è┘è┘ ╪ذ┘é┘ê╪ر',
  comfort: '╪▒╪د╪ص╪ر ╪د┘┘à┘é╪د╪╣╪»',
  quietness: '┘ç╪»┘ê╪ة ╪د┘┘à╪▒┘â╪ذ╪ر',
  safety: '╪│┘╪د┘à╪ر ╪د┘╪│┘è╪د╪▒╪ر ┘ê╪ث╪ص╪▓┘à╪ر ╪د┘╪ث┘à╪د┘',
};

const CAPTAIN_CRITERIA_LABELS: Record<string, string> = {
  behavior: '╪د┘╪د╪ص╪ز╪▒╪د┘à ┘ê╪د┘╪ث╪│┘┘ê╪ذ',
  driving: '╪د┘┘é┘è╪د╪»╪ر ╪د┘╪ت┘à┘╪ر ┘ê╪د┘╪د┘╪ز╪▓╪د┘à ╪ذ╪د┘╪│╪▒╪╣╪ر',
  punctuality: '╪د┘╪د┘╪ز╪▓╪د┘à ╪ذ┘à┘ê┘é╪╣ ╪د┘╪▒┘â┘ê╪ذ ┘ê╪د┘┘ê┘é╪ز',
  routing: '╪د╪«╪ز┘è╪د╪▒ ┘à╪│╪د╪▒ ╪░┘â┘è ╪ذ╪»┘ê┘ ╪▓╪ص╪د┘à',
  communication: '╪د┘╪ز╪ش╪د┘ê╪ذ ┘ê╪د┘╪ز┘ê╪د╪╡┘ ╪د┘╪د╪ص╪ز╪▒╪د┘┘è',
};
...
export interface HistoryTabProps {
  ...
  // ظ¤ hide them on a screen meant to be trip history specifically.
...
export function HistoryTab({ hideCaptainDiagnostics = false }: HistoryTabProps = {}) {
  ...
  const clearSovereignLogs = async () => {
    ...
    try {
      ...
      setSovereignLogs([]);
      toast({
        title: "≡اد╣ ╪ز┘à ╪ز┘╪▒┘è╪║ ╪د┘╪│╪ش┘ ╪د┘┘à╪ص┘┘è",
        description: "╪ز┘à ┘à╪│╪ص ╪ش┘à┘è╪╣ ╪│╪ش┘╪د╪ز ╪د┘╪ص╪▒┘â╪ر ┘ê╪د┘╪ص╪د┘╪د╪ز ╪د┘┘à╪ص┘┘è╪ر ┘à┘ ┘ç╪د╪ز┘┘â ╪ذ┘╪ش╪د╪ص ╪│╪د╪خ┘é."
      });
    } catch (err) {
    ...
    return (
      <div className={styles.style830_10}>
        <div className={styles.style831_11}>
          ...
          <span>{isArabic ? '╪ز┘é┘è┘è┘à┘â ╪د┘┘à┘╪╡┘ّ┘ ┘┘┘â╪د╪ذ╪ز┘:' : 'Your detailed feedback for captain:'}</span>
        </div>
    ...
    <div className={styles.style985_56}>
                      ...
                      <div className={styles.style1045_79}>
                        ...
                        <div className={styles.style1062_85}>
                          ...
                          <span className={styles.style1066_87}>
                            {isArabic ? '┘é╪ذ┘' : ''} {timeAgo === 0 ? copy.lessThanHour : `${timeAgo} ${copy.hours}`} {isArabic ? '' : 'ago'}
                          </span>
                        </div>
                      </div>
          ...
          <Card className={styles.style1092_91}>
            <CardHeader className={styles.style1093_92}>
              ...
              <Badge className={styles.style1103_96}>
                {favoriteCaptains.length} {isArabic ? '╪│╪د╪خ┘é' : 'drivers'}
              </Badge>
            </CardHeader>
            <CardContent className={styles.style1107_97}>
              {favoriteCaptains.length === 0 ? (
                <div className={styles.style1109_98}>
                  {isArabic ? (
                    <>╪د╪╢╪║╪╖ ╪╣┘┘ë ╪ث┘è┘é┘ê┘╪ر <strong className={styles.style1111_99}>╪د┘┘é┘╪ذ</strong> ┘┘è ╪ث┘è ╪▒╪ص┘╪ر ┘à┘â╪ز┘à┘╪ر ┘╪ح╪╢╪د┘╪ر ╪د┘╪│╪د╪خ┘é ╪ح┘┘ë ╪د┘┘à┘╪╢┘╪ر.</>
                  ) : (
      ...
      {isCaptain && (
        <div className={styles.style1158_112}>
          <Card className={styles.style1159_113}>
            <CardHeader className={styles.style1160_114}>
              ...
              <Badge variant="outline" className={styles.style1170_118}>
                {captainHistoricalTrips.length} {isArabic ? '┘à┘ç┘à╪ر' : 'tasks'}
              </Badge>
            </CardHeader>
            ...
            <CardContent className={styles.style1175_119}>
              {loading ? (
                ...
                <div className={styles.style1179_120}>
                  ...
                  <p className={styles.style1181_122}>{isArabic ? "┘╪د ╪ز┘ê╪ش╪» ┘à┘ç╪د┘à ┘à┘è╪»╪د┘┘è╪ر ┘à┘╪ش╪▓╪ر ┘à╪│╪ش┘╪ر ┘┘à┘╪╖┘é╪ر ╪ص╪د┘┘è╪د┘ï." : "No completed field tasks recorded for this area currently."}</p>
                </div>
              ) : (
                      ...
                      <div className={styles.style1192_124}>
                        <div>
                          <h4 className={styles.style1194_125}>
                            ≡اّج {isArabic ? '╪د┘╪▒╪د┘â╪ذ' : 'Rider'}: {trip.riderName}
                          </h4>
                          <p className={styles.style1197_126}>
                            {isArabic ? '┘à┘' : 'From'}: {trip.pickup} ظئ¤ {isArabic ? '╪ح┘┘ë' : 'To'}: {trip.dropoff}
                          </p>
                          {trip.serialId && (
                        ...
                        <div className={styles.style1208_129}>
                          ...
                          <span className={styles.style1212_131}>
                            {isArabic ? '┘é╪ذ┘' : ''} {timeAgo} {copy.hours} {isArabic ? '' : 'ago'}
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <>
          {/* ╪ذ╪▒╪د╪ة╪ر ╪░┘à╪ر ┘┘é╪د╪ة ╪د┘┘╪╕╪د┘à ╪د┘╪ص╪د┘╪ر ┘à┘ ╪د┘╪س╪▒╪س╪▒╪ر ╪د┘╪┤╪ذ┘â┘è╪ر ┘ê┘é┘┘ê╪د╪ز ╪د┘┘à╪│╪د╪╣╪»╪ر */}
              ...
              <div className={styles.style1228_135}>
                <div>
                  <CardTitle className={styles.style1230_136}>
                    ...
                    ┘ê╪س┘è┘é╪ر ╪ذ╪▒╪د╪ة╪ر ╪د┘╪░┘à╪ر  ┘ê┘┘é╪د╪ة ╪د┘┘╪╕╪د┘à (Anti-Chattiness & Zero-Chat Decree)
                  </CardTitle>
                  <CardDescription className={styles.style1234_138}>
                    ╪┤┘ç╪د╪»╪ر ┘ç┘╪»╪│┘è╪ر ┘à╪╣╪ز┘à╪»╪ر ╪ز╪س╪ذ╪ز ╪«┘┘ê ╪د┘┘╪╕╪د┘à ╪ز┘à╪د┘à╪د┘ï ┘à┘ ╪ث┘è ╪ذ╪▒┘ê╪ز┘ê┘â┘ê┘╪د╪ز ╪»╪▒╪»╪┤╪ر ┘à╪│╪د╪╣╪»╪ر ╪ث┘ê ╪د╪│╪ز┘ç┘╪د┘â ╪╣╪┤┘ê╪د╪خ┘è ┘┘╪ذ╪د┘é╪ر
                  </CardDescription>
                </div>
            ...
            <CardContent className={styles.style1243_140}>
              <div className={styles.style1244_141}>
                <div className={styles.style1245_142}>
                  ...
                  <div className={styles.style1249_145}>
                    <h5 className={styles.style1250_146}>╪╡┘╪▒ ╪ز╪╣┘é┘è╪» ┘ê╪╡┘╪▒ ╪س╪▒╪س╪▒╪ر ╪┤╪ذ┘â┘è╪ر (Zero-Chat Mandate)</h5>
                    <p className={styles.style1251_147}>
                      ┘╪د ╪ز┘ê╪ش╪» ┘é┘┘ê╪د╪ز ┘à╪ص╪د╪»╪س╪ر ╪«┘┘┘è╪ر ╪ث┘ê ╪»╪▒╪»╪┤╪ر ┘à╪╣┘é╪»╪ر ╪ز╪│╪ز┘ç┘┘â ╪ذ╪د┘é╪ر ╪د┘╪ح┘╪ز╪▒┘╪ز. ╪د┘╪ز┘ê╪د╪╡┘ ┘è╪ز┘à ╪╣╪ذ╪▒ ╪▒┘ê╪د╪ذ╪╖ ┘à╪ذ╪د╪┤╪▒╪ر ┘╪ز┘é┘┘è┘ ╪د┘╪╢╪║╪╖ ╪╣┘┘ë ╪د┘╪┤╪ذ┘â╪ر.
                    </p>
                  </div>
                </div>
                ...
                <div className={styles.style1257_148}>
                  ...
                  <div className={styles.style1261_151}>
                    <h5 className={styles.style1262_152}>╪د┘┘╪┤╪د╪╖ ╪د┘┘à╪د┘┘è ┘ê╪د┘╪▒╪ذ╪╖  ╪د┘┘à╪ج╪ز┘à╪ز</h5>
                    <p className={styles.style1263_153}>
                      ┘è╪ز┘à ╪ش┘╪ذ ╪د┘┘╪┤╪د╪╖ ┘ê╪د┘╪▒╪ص┘╪د╪ز ╪╣┘╪» ╪د┘╪ص╪د╪ش╪ر ╪د┘┘à╪ذ╪د╪┤╪▒╪ر ┘┘é╪╖ (Event-Driven) ╪»┘ê┘ ╪س╪▒╪س╪▒╪ر ╪┤╪ذ┘â┘è╪ر ┘à╪│╪ز┘à╪▒╪ر (No polling chat networks). ╪ز┘╪ز╪▓┘à ╪┤╪د╪┤╪ر ╪د┘╪│╪ش┘ ╪ذ┘à╪ذ╪»╪ث ╪د┘┘à╪ص┘â┘à ╪د┘╪▒┘é┘à┘è ╪د┘┘é╪╖╪╣┘è (SSOT) ╪ذ┘╪│╪ذ╪ر 100%.
                    </p>
                  </div>
                </div>
                ...
                <div className={styles.style1269_154}>
                  ...
                  <div className={styles.style1273_157}>
                    <h5 className={styles.style1274_158}>╪ز╪╡┘┘è╪ر ╪د┘╪│╪ش┘╪د╪ز ╪د┘┘à╪ج╪ز┘à╪ز (72-Hour Auto Purge)</h5>
                    <p className={styles.style1275_159}>
                      ╪د┘à╪ز╪س╪د┘╪د┘ï ┘┘┘à╪ص╪»╪» ╪د┘╪س╪د┘╪س ┘┘è ╪ذ┘┘ê╪» ╪د┘┘à╪╣┘à╪د╪▒┘è╪ر ╪د┘┘à╪ص┘┘è╪ر ╪د┘╪ص╪د┘╪ر╪î ┘è╪ز┘à ╪ح┘è┘é╪د┘ ┘ê┘à╪│╪ص ╪ش┘à┘è╪╣ ╪ز┘╪د╪╡┘è┘ ╪د┘╪ص╪▒┘â╪ر ┘à┘è┘â╪د┘┘è┘â┘è╪د┘ï ╪ذ╪╣╪» ┘à╪▒┘ê╪▒ 72 ╪│╪د╪╣╪ر ╪س╪د╪ذ╪ز╪ر ┘à┘ ┘ç╪د╪ز┘┘â ┘ê╪«╪د╪»┘à ╪د┘┘╪╕╪د┘à ┘╪ص┘à╪د┘è╪ر ╪«╪╡┘ê╪╡┘è╪ز┘â ┘ê╪╡┘╪▒┘è╪ر ╪د┘╪ز┘â┘┘╪ر ╪د┘╪ص╪د┘╪ر.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className={styles.style1282_160}>
                <div className={styles.style1283_161}>
                  ...
                  <span className={styles.style1285_163}>╪ص╪د┘╪ر ┘┘é╪د╪ة ╪«╪╖┘ê╪╖ ╪د┘┘┘é┘ ╪د┘╪ص╪د┘┘è╪ر:</span>
                  <span className={styles.style1286_164}>100% PURE & SECURE</span>
                </div>
                <span className={styles.style1288_165}>╪ذ╪▒╪د╪ة╪ر ╪░┘à╪ر ┘à╪╣╪ز┘à╪»╪ر ┘ê┘à╪«╪ز┘ê┘à╪ر ╪▒┘é┘à┘è╪د┘ï ≡اؤةي╕</span>
              </div>
            </CardContent>
          </Card>

          {/* Dedicated Sovereign Logs (╪│╪ش┘ ╪«╪د╪╡ ╪ذ┘ç ┘ê┘è┘â┘ê┘ ┘à╪▒╪ش╪╣┘ï╪د ┘┘ç) */}
            ...
            <CardHeader className={styles.style1296_168}>
              <div>
                <CardTitle className={styles.style1298_169}>
                  ...
                  ╪│╪ش┘ ╪د┘┘╪╣╪د┘┘è╪د╪ز ┘ê╪د┘╪ص╪▒┘â╪ر ╪د┘┘╪د┘à╪▒┘â╪▓┘è╪ر (╪د┘╪ث╪▒╪┤┘è┘ )
                </CardTitle>
                <CardDescription className={styles.style1302_171}>
                  ╪│╪ش┘ ┘é╪╖╪د╪╣ ╪د┘┘╪د┘é┘ ╪د┘┘à┘è╪»╪د┘┘è ╪د┘╪░╪د╪ز┘è ┘┘à╪▒╪د┘é╪ذ╪ر ╪ز╪ذ╪»┘è┘ ╪د┘╪ص╪د┘╪ر ┘ê┘à╪ص┘è╪╖ ╪د┘┘à┘╪╖┘é╪ر
                </CardDescription>
              </div>
            ...
            <CardContent className={styles.style1324_176}>
              {loading ? (
                ...
                <div className={styles.style1328_177}>
                  ...
                  <p className={styles.style1330_179}>╪د┘╪│╪ش┘ ╪«╪د┘ê┘ ╪ص╪د┘┘è╪د┘ï ╪│╪د╪خ┘é.</p>
                  <p className={styles.style1331_180}>
                    ╪│┘è╪ز┘à ╪ز┘┘é╪د╪خ┘è╪د┘ï ╪ز╪«┘┘è╪» ╪د┘╪ص╪▒┘â╪د╪ز ╪د┘┘à┘è╪»╪د┘┘è╪ر ┘à╪س┘ ╪ز╪ذ╪»┘è┘ ╪د┘╪ص╪د┘╪ر ╪ذ┘è┘ ╪د┘┘╪┤╪╖ ┘ê╪د┘╪«╪د┘à┘╪î ╪د┘╪ز╪╣╪╖┘è┘ ╪د┘╪ز┘┘é╪د╪خ┘è ╪ذ╪│╪ذ╪ذ ╪د┘╪«┘à┘ê┘ ╪ث┘ê ┘┘╪د╪» ╪د┘╪ذ╪د┘é╪ر╪î ┘ê╪«╪▒┘ê╪ش┘â ┘à┘ ┘à╪ص┘è╪╖ ╪د┘┘à┘╪╖┘é╪ر ┘ç┘╪د ┘â┘à╪▒╪ش╪╣  ╪ت┘à┘ ┘ê╪ث┘à┘┘è ┘┘â.
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div className={styles.style1353_183}>
                          <span className={cn(styles.style1354_184, badgeColor)}>
                            {iconEmoji} {log.type === 'status_change' ? '╪ز╪╣╪»┘è┘ ╪د┘╪ص╪د┘╪ر' : log.type === 'system_action' ? '╪ح╪ش╪▒╪د╪ة ╪د┘┘╪╕╪د┘à' : '╪ز╪«╪╖┘è ╪د┘╪ص╪»┘ê╪»'}
                          </span>
          ...
          {/* ┘â╪┤╪د┘ ╪د┘┘é╪د┘à┘ê╪│  ┘┘╪ث╪«╪╖╪د╪ة (SSOT Error Explorer) */}
            ...
            <CardHeader className={styles.style1380_191}>
              <div className={styles.style1381_192}>
                <div>
                  ...
                  <CardDescription className={styles.style1387_195}>
                    ╪ث╪»╪د╪ر ┘╪ص╪╡ ╪ز┘╪د╪╣┘┘è╪ر ┘╪▒┘à┘ê╪▓ ╪د┘╪ث┘à╪د┘ ┘ê╪د┘┘à╪ص┘â┘à ╪د┘┘à┘è╪»╪د┘┘è ╪د┘╪ص╪د┘╪ر
                  </CardDescription>
                </div>
              ...
              <div className={styles.style1397_197}>
                <div className={styles.style1398_198}>
                  ...
                  <input
                    type="text"
                    placeholder="╪د╪ذ╪ص╪س ╪ذ┘â┘ê╪» ╪د┘╪«╪╖╪ث╪î ╪د┘╪د╪│┘à╪î ╪ث┘ê ╪د┘╪ح╪ش╪▒╪د╪ة ╪د┘┘ê┘é╪د╪خ┘è..."
                    value={errorSearch}
            ...
            <CardContent className={styles.style1462_220}>
              {filteredErrors.length === 0 ? (
                <div className={styles.style1464_221}>
                  ...
                  <p className={styles.style1466_223}>┘╪د ╪ز┘ê╪ش╪» ╪▒┘à┘ê╪▓ ╪ث╪«╪╖╪د╪ة ╪ز╪╖╪د╪ذ┘é ╪ذ╪ص╪س┘â ╪ص╪د┘┘è╪د┘ï.</p>
                </div>
              ) : (
                        ...
                        {isExpanded && (
                          <div className={styles.style1517_239}>
                            <div className={styles.style1518_240}>
                              <span className={styles.style1519_241}>╪د┘┘ê╪╡┘ ╪د┘╪ث┘à┘┘è ┘┘╪«┘┘:</span>
                              <p className={styles.style1520_242}>
                                {err.description}
                              </p>
                            </div>
                            <div className={styles.style1524_243}>
                              <span className={styles.style1525_244}>≡اؤةي╕ ╪د┘╪ح╪ش╪▒╪د╪ة ╪د┘┘ê┘é╪د╪خ┘è ╪د┘╪ت┘┘è ╪د┘╪ص╪د┘╪ر:</span>
...
const historyLanguageCopy = {
  ar: {
    title: '╪▒╪ص┘╪د╪ز┘è',
    subtitle: '╪▒╪د╪ش╪╣ ╪▒╪ص┘╪د╪ز┘â ╪د┘╪ث╪«┘è╪▒╪ر. ┘è╪ز┘à ╪ص╪░┘ ╪د┘╪▒╪ص┘╪د╪ز ╪د┘╪ز┘è ┘à╪▒ ╪╣┘┘è┘ç╪د ╪ث┘â╪س╪▒ ┘à┘ 72 ╪│╪د╪╣╪ر ┘╪ص┘à╪د┘è╪ر ╪«╪╡┘ê╪╡┘è╪ز┘â.',
    recentTripsTitle: '╪د┘╪▒╪ص┘╪د╪ز ╪د┘╪ث╪«┘è╪▒╪ر (╪ت╪«╪▒ 3 ╪ث┘è╪د┘à)',
    recentTripsDesc: '╪د┘╪▒╪ص┘╪د╪ز ╪د┘╪ز┘è ╪د┘â╪ز┘à┘╪ز ┘à┘ ╪ص╪│╪د╪ذ┘â',
    tripCount: '╪▒╪ص┘╪ر',
    noTrips: '┘╪د ╪ز┘ê╪ش╪» ╪▒╪ص┘╪د╪ز ┘╪┤╪╖╪ر ┘à╪│╪ش┘╪ر ┘┘è ╪ت╪«╪▒ 72 ╪│╪د╪╣╪ر.',
    before: '┘é╪ذ┘',
    lessThanHour: '╪ث┘é┘ ┘à┘ ╪│╪د╪╣╪ر',
    hours: '╪│╪د╪╣╪ر',
    callCaptain: '╪د╪ز╪╡╪د┘ ╪ذ╪د┘╪│╪د╪خ┘é ╪ذ╪«╪╡┘ê╪╡ ╪د┘╪▒╪ص┘╪ر',
    savedCaptainsTitle: '╪د┘╪│╪د╪خ┘é┘ê┘ ╪د┘┘à┘╪╢┘┘ê┘',
    savedCaptainsDesc: '╪د┘╪│╪د╪خ┘é┘ê┘ ╪د┘╪░┘è┘ ╪ص┘╪╕╪ز┘ç┘à ┘à┘ ╪▒╪ص┘╪د╪ز┘â ╪د┘╪│╪د╪ذ┘é╪ر',
    call: '╪د╪ز╪╡┘',
    captainSectionTitle: '╪│╪ش┘ ╪د┘╪╣┘ê╪د╪خ╪» ┘ê╪د┘┘à┘ç╪د┘à ╪د┘┘à┘è╪»╪د┘┘è╪ر ╪د┘┘à┘╪ش╪▓╪ر',
    captainSectionDesc: '╪د┘┘à┘ç╪د┘à ╪د┘┘à╪╣╪ز┘à╪»╪ر ╪د┘┘à┘ê╪س┘é╪ر ╪ذ┘à╪▒┘â╪▓ ╪د┘┘╪┤╪د╪╖',
    commission: '╪د┘╪╣┘à┘ê┘╪ر ╪د┘┘à╪│╪ز╪ص┘é╪ر',
    status: '╪د┘╪ص╪د┘╪ر',
    active: '┘╪┤╪╖',
    pending: '┘é┘è╪» ╪د┘╪د┘╪ز╪╕╪د╪▒',
  },
