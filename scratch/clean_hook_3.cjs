...
// Remove renderDetailedReview by finding start and end
const renderReviewStart = hookBody.indexOf('const renderDetailedReview = (tripId: string) => {');
if (renderReviewStart !== -1) {
  const renderReviewEnd = hookBody.indexOf('};\n\n', renderReviewStart);
  if (renderReviewEnd !== -1) {
    hookBody = hookBody.substring(0, renderReviewStart) + hookBody.substring(renderReviewEnd + 4);
  } else {
    const backupEnd = hookBody.indexOf('};', renderReviewStart);
    hookBody = hookBody.substring(0, renderReviewStart) + hookBody.substring(backupEnd + 2);
  }
}
