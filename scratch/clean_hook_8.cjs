...
// NOW WE REGEX OUT renderDetailedReview on the final string
let lines = finalHook.split('\\n');
let newLines = [];
let skip = false;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('const renderDetailedReview')) {
    skip = true;
  }
  if (!skip) {
...
fs.writeFileSync('src/features/account/components/history-tab/use-history-state.ts', newLines.join('\\n'));
console.log('Hook rewritten cleanly, with explicit line filtering!');
