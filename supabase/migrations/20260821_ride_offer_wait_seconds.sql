-- Adds a per-offer visibility window: the captain picks how many seconds
-- their offer stays shown to the rider before it auto-hides if unanswered.
-- Minimum enforced both client-side and here (server-authoritative).

alter table if exists public.ride_offers
  add column if not exists wait_seconds integer not null default 5;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'ride_offers_wait_seconds_min'
  ) then
    alter table public.ride_offers
      add constraint ride_offers_wait_seconds_min check (wait_seconds >= 5);
  end if;
end $$;

drop function if exists public.submit_ride_offer(uuid, numeric);
