-- Migration: Fix 22P02 invalid input value for enum ride_offer_status in get_passenger_bids & set_captain_status

-- 1. Update get_passenger_bids with explicit text casting for enum status
...
-- 2. Update set_captain_status with explicit text casting for enum status
...
BEGIN
  ...
  IF requested_status = 'idle' OR NOT public.is_active_captain_status(resolved_status) THEN
    ...
    WHERE captain_id = auth.uid() 
      AND (status IS NULL OR status::text = 'PENDING' OR status::text = 'pending');
