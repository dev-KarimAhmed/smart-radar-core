-- Migration: 20260903230000_refund_minutes_on_trip_cancel.sql
-- Description: Automatically refund charged radar minutes to captain wallet if trip is CANCELLED.
...
BEGIN
  ...
  v_captain := COALESCE(NEW.accepted_captain_id, OLD.accepted_captain_id);
  v_already_charged := COALESCE(OLD.wallet_minutes_charged, NEW.wallet_minutes_charged, 0);

  -- 1. Handling CANCELLED status: Refund all previously charged minutes back to the captain
  IF v_status = 'CANCELLED' THEN
    IF v_already_charged > 0 AND v_captain IS NOT NULL THEN
      BEGIN
        UPDATE public.wallet_accounts
        SET paid_minutes_remaining = coalesce(paid_minutes_remaining, 0) + v_already_charged,
            updated_at = now()
        ...
        VALUES (
          v_captain, 'trip_time_refund', 'trip_time_refund', 0, 'COMPLETED',
          format('استرداد %s دقيقة رادار بسبب إلغاء الرحلة.', v_already_charged),
          jsonb_build_object(
            'requestId', NEW.id,
            'minutesRefunded', v_already_charged,
            'status', v_status,
            'cancelledAt', now()
          )
        );
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING '[charge_wallet_for_trip_time refund] request % captain %: % (%)',
  ...
  -- 2. Deduct when trip status becomes ACCEPTED, ARRIVED, STARTED, or COMPLETED
